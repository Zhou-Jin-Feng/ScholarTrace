...
uv run scholartrace-backup `
  ...
  --output backups/scholartrace-1.0.2-20260919.zip
```
...
uv run scholartrace-restore `
  --archive backups/scholartrace-1.0.2-20260919.zip `
...
docker compose run --rm --no-deps `
  ...
  --output /backup/scholartrace-1.0.2-20260919.zip
```
...
docker compose run --rm --no-deps `
  ...
  --archive /backup/scholartrace-1.0.2-20260919.zip `
...
5. 当前 `1.0.2` 没有自动数据库迁移器；版本不一致时不得强行恢复；
